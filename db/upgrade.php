...
function xmldb_local_intellistream_upgrade($oldversion) {
    ...
    if ($oldversion < 2026082000) {
        ...
        // that does not mutate core configuration on 4.3+ — see
        // \local_intellistream\helpers\custom_menu_helper.
        //
        // Nothing to restore here: 2026080310 deleted the stored value, and it is
        // not recoverable. Sites that had the toggle on must switch it back on.
        // Seeding is what makes the new key present rather than merely defaulted,
        // matching every other declared default; it only writes absent keys.
        $seeded = \local_intellistream\config::seed_declared_defaults();
        if ($seeded) {
            mtrace('local_intellistream: seeded missing setting defaults: ' . implode(', ', $seeded));
        }
